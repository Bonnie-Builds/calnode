...
// account email but NO tokens â€” custody mode never decrypts local material.
func seedDestinationConnection(t *testing.T, c *Client, userID, accountEmail string) {
	t.Helper()
	seedUser(t, c.db, userID)
	_, err := c.db.ExecContext(context.Background(), `UPDATE users SET email = ? WHERE id = ?`, accountEmail, userID)
	if err != nil {
...
func TestCustodyCreateEvent_envelopeCarriesExactContractShape(t *testing.T) {
	...
	if req.MemberSub != "member@workspace.example.com" {
		t.Errorf("memberSub = %q; want managed member identity", req.MemberSub)
	}
...
func TestCustodyAmbiguousUpsert_convergesViaEventGetWithoutDuplicateEffect(t *testing.T) {
	// Script: upsert â†’ ambiguous; get â†’ event already exists. A later heal retry
