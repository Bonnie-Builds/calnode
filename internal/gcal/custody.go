...
// destinationMemberSub resolves the provisioned member email for the user's
// destination connection row. No token material is read or decrypted.
func (c *Client) destinationMemberSub(ctx context.Context, userID string) (string, bool) {
	...
	err := c.db.QueryRowContext(ctx,
		`SELECT COALESCE(account_email,'') FROM calendar_connections
		 WHERE user_id = ? AND provider = 'google' AND is_destination = 1 LIMIT 1`, userID).Scan(&email)
...
func (c *Client) custodyFreeBusyConnections(ctx context.Context, userID string) ([]custodyConnRow, error) {
	rows, err := c.db.QueryContext(ctx, `
		SELECT COALESCE(account_email,''), calendar_id
		FROM calendar_connections
		WHERE user_id = ? AND provider = 'google' AND check_conflicts = 1`, userID)
	if err != nil {
		return nil, fmt.Errorf("gcal: load custody freebusy connections: %w", err)
	}
	defer rows.Close()
	var out []custodyConnRow
	for rows.Next() {
		var r custodyConnRow
		if err := rows.Scan(&r.accountEmail, &r.calID); err != nil {
			return nil, fmt.Errorf("gcal: scan custody freebusy connection: %w", err)
		}
		if strings.TrimSpace(r.accountEmail) == "" {
			// A row without a provisioned member email cannot bind sub; skip
			// (fail-open, same posture as a flaky direct connection).
			c.logger.Warn("custody: skipping conflict connection without provisioned member email", "user_id", userID)
			continue
		}
		r.accountEmail = strings.ToLower(strings.TrimSpace(r.accountEmail))
		out = append(out, r)
	}
	return out, rows.Err()
}
...
func (c *Client) custodyFreeBusy(ctx context.Context, userID string, from, to time.Time) ([]slots.Interval, error) {
	...
	for _, conn := range conns {
		calIDs, err := calendar.ConflictCalendarIDs(ctx, c.db, "google", userID, conn.accountEmail, conn.calID)
		if err != nil {
